...
class ComputeMatGrad:
    ...
    def conv2d(input, grad_output, layer):
        ...
        """
        with torch.no_grad():
            input = _extract_patches(input, layer.kernel_size, layer.stride,
                                     layer.padding)
            ...
            if layer.bias is not None:
                input = torch.cat(
                    [input, input.new(input.size(0), 1).fill_(1)], 1)
            input = input.view(grad_output.size(0), -1,
                               input.size(-1))  # b * hw * in_c*kh*kw
...
class KFAC(optim.Optimizer):
    def __init__(self,
        ...
        if damping_warmup_steps == 0:
            ...
            self.damping_factor = 2 * math.log10(
                damping_scale) / damping_warmup_steps
        self.damping_min = damping
    ...
    def adjust_damping(self, damping):
        return (1 - self.damping_factor
                ) * damping + self.damping_factor * self.damping_min
    ...
    def _save_grad_output(self, module, grad_input, grad_output):
        ...
        if self.acc_stats and self.steps % self.TCov == 0:
            gg = self.CovGHandler(grad_output[0].data, module,
                                  self.batch_averaged)
            # Initialize buffers
            if self.steps == 0:
    ...
    def _update_inv(self, m):
        ...
        self.d_a[m], self.Q_a[m] = torch.linalg.eigh(
            self.m_aa[m])  # torch.symeig(self.m_aa[m], eigenvectors=True)
        self.d_g[m], self.Q_g[m] = torch.linalg.eigh(
            self.m_gg[m])  # torch.symeig(self.m_gg[m], eigenvectors=True)
    ...
    def step(self, closure=None):
        ...
        lr = group['lr']
        if self.steps != 0 and self.steps % self.TInv == 0:
            group['damping'] = self.adjust_damping(group['damping'])
